...
/*控制接口*/
	...
	//垂直加速度误差TD4滤波器
	static Filter_Butter2_LP AccZ_Err_filter;
	static double accZErrD_filted = 0;
	...
	/*高度*/
		...
		bool Altitude_Control_Enable( double TIMEOUT )
		{
			...
			if( LockCtrl(TIMEOUT) )
			{
				...
				AccZ_Err_filter.set_cutoff_frequency( CtrlRateHz, cfg.ZSense[0] );
				accZErrD_filted = 0;
...
PosCtrl_Finish:	
	...
	{//高度控制
		...
		if( inFlight )
		{
			...
			double last_accZErr = AccZ_Err_filter.get_result();
			acceleration_z_error = AccZ_Err_filter.run( acceleration_z_error );
			double accZErrD = ( acceleration_z_error - last_accZErr ) * CtrlRateHz;
			double correction = h*( Pa*acceleration_z_error );
			AccZ_Err_filter.add_offset( -1*correction, 0 );
			//加速度误差导数滤波
			accZErrD_filted += 2*Pi*cfg.ZSenseD[0] * h * ( accZErrD - accZErrD_filted );
			...
			if( force < 1 )
				force = 1;
			double throttle = ( force + T * ( Pa*acceleration_z_error + accZErrD_filted + target_acceleration_z_1 ) )/b;
