...
// HTL使能
extern "C" __attribute__((section(".BKSRAM"))) uint32_t HTL_NeedEna = 0;
static bool HTL_ena = false;
extern "C" bool HTL_isEnabled()
{
    return HTL_ena;
}
...
{	
  ...
  /* Enable D-Cache---------------------------------------------------------*/
	...
	//打开SRAM2用作SD缓冲区
	RCC->AHB2ENR |= (1<<30) | (1<<31);
	
	// 开启backup
//	PWR->CR2 |= (1<<0);
//	while( ( PWR->CR2 & (1<<16) ) == 0 );
//	//打开BKSRAM
//	RCC->AHB4ENR |= (1<<28);
  ...
  HAL_PWR_EnableBkUpAccess();
	...
	// HTL使能
	if (HTL_NeedEna == 0xac1234ac)
	{
			HTL_ena = true;
			HTL_NeedEna = 0;
	}
	
	LPDaemon_Queue = xQueueCreate( 30, sizeof(LPDaemonTaskInf) );
