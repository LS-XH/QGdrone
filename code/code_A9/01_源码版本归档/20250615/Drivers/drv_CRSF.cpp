...
static void send_ATTITUDE(const Port* port)
{
	struct PACKET
	{
		CRSF_HEADER header;
	...
	PACKET packet;
	packet.header.header = 0xc8;
	packet.header.size = sizeof(PACKET)-2;
	packet.header.type = CRSF_FRAMETYPE_ATTITUDE;
	
	Quaternion quat;
	get_AirframeY_quat(&quat);
	quat.Enu2Ned();
	packet.pit = rad2degree(quat.getPitch()) * 100;
	packet.pit = __REV16(packet.pit);
	packet.rol = rad2degree(quat.getRoll()) * 100;
	packet.rol = __REV16(packet.rol);
	packet.yaw = rad2degree(quat.getYaw()) * 100;
	packet.yaw = __REV16(packet.yaw);
	
	packet.crc = crc8((const uint8_t *)&packet.header.type, packet.header.size - 1);
	
	port->write( (uint8_t*)&packet, sizeof(PACKET), 0.1, 0.1 );
}
static void send_FrameBatterySensor(const Port* port)
{
	struct PACKET
	{
		CRSF_HEADER header;
	...
	packet.header.header = 0xc8;
	packet.header.size = sizeof(PACKET)-2;
	packet.header.type = CRSF_FRAMETYPE_BATTERY_SENSOR;
	
	BatteryInfo batInfo;
	getCurrentBatteryInfo(&batInfo);
	packet.volt = batInfo.totalVolt * 0.1f;
	packet.volt = __REV16(packet.volt);
	packet.current = batInfo.totalCurrent * 0.1f;
	packet.current = __REV16(packet.current);
	packet.capacity = batInfo.totalPowerUsage * 1000 / batInfo.stVolt;
	packet.capacity = __REV(packet.capacity) >> 8;
	packet.batRemain = batInfo.totalPercent;
	packet.batRemain = __REV16(packet.batRemain);
...
static void CRSF_Server(void* pvParameters)
{
	...
	// 发送状态机
	TIME last_sdTIME(false);
	uint8_t sd_counter = 0;
	
	while(1)
	{	
		uint8_t rdata;
		if( driver_info.port.read( &rdata, 1, 2, 0.1 ) ) 
		{
			if(rc_counter == 0)
			{	//目标设备 0xC8为飞控
				if(rdata == 0xC8)
			...
			else 
			{
				...
				uint8_t crc = crc8((const uint8_t *)&crsf_rc_channels.type, 23);
		...
		if( last_sdTIME.get_pass_time() > 0.5 )
		{
			last_sdTIME = TIME::now();
			
			switch(sd_counter)
			{
				case 0:
				{
					send_ATTITUDE(&driver_info.port);
					++sd_counter;
					break;
				}
				default:
				case 1:
				{
					send_FrameBatterySensor(&driver_info.port);
					sd_counter = 0;
					break;
				}
			}
