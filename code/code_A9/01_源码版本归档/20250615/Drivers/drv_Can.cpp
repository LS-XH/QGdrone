...
/*CanMailBox*/
	static list<CanMailBox*> mailBoxes;
	//邮箱列表访问互斥锁
	static SemaphoreHandle_t mailBoxesMutex; 
	static inline bool lock_mailBoxes( double Sync_waitTime )
	{
		...
		if( xSemaphoreTakeRecursive( mailBoxesMutex , Sync_waitTicks ) == pdTRUE )
			return true;
		return false;
	}
	static inline void unlock_mailBoxes()
	{
		xSemaphoreGiveRecursive( mailBoxesMutex );
	}
	
	bool CanMailBox::checkMail(CanId id)
	{
		for( uint16_t i=0; i<this->rx_idCount; ++i )
		{
			if( id.IdType )
			{	//扩展帧
				if( this->ids[i] < 0x20000000 )
				{
					if( (id.Identifier & this->id_masks[i]) == (this->ids[i] & this->id_masks[i]) )
						return true;
				}
				else
					continue;
			...
			{	//标准帧
				if( this->ids[i] >= 0x20000000 )
				{
					if( (id.Identifier & this->id_masks[i]) == ((this->ids[i]-0x20000000) & this->id_masks[i]) )
						return true;
				}
				else
					continue;
	...
	CanMailBox::CanMailBox( uint16_t Capacity, 
								CanIdWithMask* Ids, uint16_t Ids_count )
	{
		if( Ids && Ids_count )
		{
			this->rx_idCount = Ids_count;
			this->ids = new uint32_t[this->rx_idCount];
			this->id_masks = new uint32_t[this->rx_idCount];
			for( uint16_t i = 0; i < this->rx_idCount; ++i )
			{	//Id-MailBox关系放置在Map中
				...
				{	//标准帧
					if(Ids[i].mask)
						...
						mask = 0xffffffff;
					id = (Ids[i].Identifier & mask) + 0x20000000;
				}
				
				this->ids[i] = id;
				this->id_masks[i] = mask;
			}
			packet_queue = xQueueCreate( Capacity, sizeof(CanPacket) );
			
			lock_mailBoxes(-1);
			mailBoxes.push_back(this);
			unlock_mailBoxes();
		}
		else
		{
								...
								CanId* Ids, uint16_t Ids_count )
	{
		if( Ids && Ids_count )
		{
			...
			{	//Id-MailBox关系放置在Map中
				...
				uint32_t mask;
				if( Ids[i].IdType )
					id = Ids[i].Identifier;	//扩展帧
				else
					id = Ids[i].Identifier + 0x20000000;	//标准帧
				mask = 0xffffffff;
	...
	void CanMailBox::CanRxTCB( void *pvParameter1, uint32_t ulParameter2 )
	{
		...
		for( uint8_t i = 0; i < 5; ++i )
		{
			...
			CanId canId;
			canId.Identifier = RxHeader.Identifier;
			canId.IdType = RxHeader.IdType == FDCAN_STANDARD_ID ? 0 : 1;
			
			lock_mailBoxes(-1);
			for (std::list<CanMailBox*>::iterator it = mailBoxes.begin(); it != mailBoxes.end(); ++it)
			{
        if( (*it)->checkMail(canId) )
				{
					...
					BaseType_t res = xQueueSend( (*it)->packet_queue, &packet, 0 );
					if( res != pdTRUE )
						++(*it)->mail_overflow_cnt;
				}
			}
			unlock_mailBoxes();
		}
...
void init_drv_Can(void)
{
	...
	mailBoxesMutex = xSemaphoreCreateRecursiveMutex();
