...
//			WindDisturbance_y += lp_factor * ( WindDisturbance_xy.y - WindDisturbance_y );
	...
	void update_ESO_1()
	{
		...
		if( Attitude_Control_Enabled )
		{
			...
			vector3<double> vec = quat.get_RP_quat().reverse_rotate_axis_z();
			CtrlM_ESO[0]->run( +vec.y, angular_rate.x, inFlight );
			CtrlM_ESO[1]->run( -vec.x, angular_rate.y, inFlight );
			CtrlM_ESO[2]->run( 0, angular_rate.z, inFlight );
		}
	...
	/*多旋翼*/
		...
		static void MultiRotor_Init()
		{
			...
			((ESO_AngularRate*)CtrlM_ESO[0])->init( 
				(cfg.Mode[0] & ATTCTRL_MODE_RP_LEARN_DIS_BIT) ? false : true,
			...
			((ESO_AngularRate*)CtrlM_ESO[2])->init( 
				(cfg.Mode[0] & ATTCTRL_MODE_Y_LEARN_DIS_BIT) ? false : true,
...
void init_Ctrl_Attitude()
{
	...
	cfg.Mode[0] = ATTCTRL_MODE_TURNCOMP_ENA_BIT | (1<<0);
	cfg.STDelay[0] = 3.5;
	cfg.T[0] = 0.1;
	cfg.T2[0] = 0.1;
