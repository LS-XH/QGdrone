...
enum UAVType
{
	/*10-99多旋翼*/
		...
		UAVType_Rotor6_H = 41 ,	//六旋翼H型
	
		UAVType_Rotor62_H = 51 ,	//六旋翼Double H型
	
		UAVType_Rotor62_H2 = 71 ,	//六旋翼Double H打横型
...
/*参数接口*/
	...
	struct AttCtrlCfg
	{
		...
		int32_t YawTDOrder[2];	//偏航跟踪器阶数
		float upThrRatio[2];    // 上层电机油门比例
	...
	/*姿态模式位定义*/
		...
		//关闭电压补偿
		#define ATTCTRL_MODE_BATCOMP_DIS_BIT (1<<10)
...
/*姿态控制*/
	...
	//设定目标Roll Pitch角速率
	bool Attitude_Control_set_Target_RollPitchRate( double RollRate, double PitchRate, double TIMEOUT = -1 );
...
/*位置控制*/
	...
	/*高度*/
		...
		//获取目标垂直速度
		bool Position_Control_get_TargetVelocityZ( double* velz, double TIMEOUT = -1 );
		//设定目标垂直速度
