...
/*外置罗盘定义*/
	...
	/*     名称     , iic地址, 数据寄存器地址 , 高位在前, 手动采样 , 采样寄存器地址, 采样设置, ID 地址 , ID   ,      配置寄存器地址      , 配置数目,  配置                                                     ,  灵敏度      , 采样率毫秒  , 轴向                 ,  */
		...
		{ "IST8310"   , 0x0F   ,      3         ,   false ,    true  ,          0xa  ,      1  ,      0  , 0x10 ,      { 0x41, 0x42 }      , 2       , { 0x24 , 0xc0 }                                           , 0.003        , 30          , { -1 , 2 , 3 }    } , //IST8310
...
ScanExtMag:
	...
	while(1)
	{
		for( int8_t i = 0; i < Supported_Internal_MagnetoMeter_Count; ++i )
		{
			...
			res = ExtIIC_SendReceiveAddr7( sensor->device_address, tx_buf, 1, rx_buf, 1 );
			if(!res)
				continue;
			...
			//已成功识别到传感器
			key = IMUMagnetometerRegister( Internal_Magnetometer_Index, sensor->name, sensor->sensitivity );
			if( key )
			{
...
//			else if( IMUMagnetometerRegister( 1, sensor->name, sensor->sensitivity ) )
//			{
//				magnetometer_ind = 1;
//				current_ExtMag = i;
//				goto ExtMagDetected;
...
ExtMagDetected:
	...
	while(1)
	{
		...
		do
		{
			...
			if( ++rt > 3 )
			{
				IMUMagnetometerUnRegister(magnetometer_ind,key);
				goto ScanExtMag;
			}
			//读取数据
		...
		/*更新传感器数据*/						
			...
			//更新数据
			if( sample_delay >= 50 )
				IMUMagnetometerUpdate( magnetometer_ind,key, data, false);
			else
				++sample_delay;
		/*更新传感器数据*/
	}
