...
static uint32_t registered_imuGyro[IMU_Sensors_Count] = {0};
static uint32_t registered_imuAccel[IMU_Sensors_Count] = {0};
static uint32_t registered_imuMag[IMU_Sensors_Count] = {0};
static void Msg202_ACFlyHTL_IMU_REGISTER(uint8_t Port_index, const mavlink_message_t *msg)
{
    if (HTL_isEnabled())
    {
        const mavlink_acflyhtl_imu_register_t *msg_rd = (mavlink_acflyhtl_imu_register_t *)msg->payload64;
        if (msg_rd->target_system == 0 ||
            (msg_rd->target_system == get_CommulinkSysId() && msg_rd->target_component == get_CommulinkCompId()) ||
            (msg_rd->target_system == get_CommulinkSysId() && msg_rd->target_component == 0))
        {
            switch (msg_rd->imu_type)
            {
                case GYRO0:
                case GYRO1:
                case GYRO2:
                {
                    uint8_t sId = msg_rd->imu_type - GYRO0;
                    registered_imuGyro[sId] =
                        IMUGyroscopeRegister(sId, msg_rd->sensor_name, msg_rd->sensitivity);
                    if (registered_imuGyro[sId])
                        set_IMU_Gyroscope_UpdateFreq(sId, msg_rd->freq);
                    break;
                }
                ...
                case ACCEL2:
                {
                    uint8_t sId = msg_rd->imu_type - ACCEL0;
                    registered_imuAccel[sId] =
                        IMUAccelerometerRegister(sId, msg_rd->sensor_name, msg_rd->sensitivity);
                    if (registered_imuAccel[sId])
                        set_IMU_Accelerometer_UpdateFreq(sId, msg_rd->freq);
                    break;
                }
                ...
                case COMPASS2:
                {
                    uint8_t sId = msg_rd->imu_type - COMPASS0;
                    registered_imuMag[sId] =
                        IMUMagnetometerRegister(sId, msg_rd->sensor_name, msg_rd->sensitivity);
                    break;
                }
            }
        }
...
static void Msg203_ACFlyHTL_IMU_UPDATE(uint8_t Port_index, const mavlink_message_t *msg)
{
    if (HTL_isEnabled())
    {
        const mavlink_acflyhtl_imu_update_t *msg_rd = (mavlink_acflyhtl_imu_update_t *)msg->payload64;
        if (msg_rd->target_system == 0 ||
            (msg_rd->target_system == get_CommulinkSysId() && msg_rd->target_component == get_CommulinkCompId()) ||
            (msg_rd->target_system == get_CommulinkSysId() && msg_rd->target_component == 0))
        {
            ...
            switch (msg_rd->imu_type)
            {
                ...
                {
                    uint8_t sId = msg_rd->imu_type - GYRO0;
                    vector3<int32_t> s_data;
                    s_data.x = msg_rd->x;
                    s_data.y = msg_rd->y;
                    s_data.z = msg_rd->z;
                    bool res = IMUGyroscopeUpdate(sId, registered_imuGyro[sId], s_data, false);
                    if (res)
                        MS_Notify_IMUGyroUpdate(sId);
                    break;
                }
                ...
                case ACCEL2:
                {
                    ...
                    s_data.y = msg_rd->y;
                    s_data.z = msg_rd->z;
                    bool res = IMUAccelerometerUpdate(sId, registered_imuAccel[sId], s_data, false);
                    if (res)
                        MS_Notify_IMUAccelUpdate(sId);
                    break;
                }
                ...
                case COMPASS2:
                {
                    ...
                    s_data.y = msg_rd->y;
                    s_data.z = msg_rd->z;
                    IMUMagnetometerUpdate(sId, registered_imuMag[sId], s_data, false);
                    break;
                }
            }
        }
        ...
        (msg_rd->target_system == get_CommulinkSysId() && msg_rd->target_component == 0))
    {
        ...
        { // 可以注册传感器
            ...
            { // 注册成功
                ...
                registered_sensors[msg_rd->ind].type = (Position_Sensor_Type)msg_rd->type;
                registered_sensors[msg_rd->ind].dataType = (Position_Sensor_DataType)msg_rd->DataType;
            }
        }
...
static void Msg209_ACFly_UpdatePosSensor(uint8_t Port_index, const mavlink_message_t *msg)
{
        ...
        (msg_rd->target_system == get_CommulinkSysId() && msg_rd->target_component == 0))
    {
        ...
        { // 传感器已注册
            ...
            // 传感器类型
            if( msg_rd->DataType >= 0 )
            {
                uint8_t dataType = msg_rd->DataType;
                if (PositionSensorChangeDataType(msg_rd->ind, key, (Position_Sensor_DataType)dataType))
                {
                    registered_sensors[msg_rd->ind].dataType = (Position_Sensor_DataType)dataType;
                }
            }
...
//                    iniYaw += 2 * Pi;
            ...
            switch (registered_sensors[msg_rd->ind].dataType)
            {
                ...
                case Position_Sensor_DataType_s_z_nAC:
                {
                    ...
                    PositionSensorUpdatePosition(msg_rd->ind, key, pos,
                        available, msg_rd->delay,
                        msg_rd->trustXY, msg_rd->trustZ,
                        0,
                        msg_rd->LTTrustXY, msg_rd->LTTrustZ);
                    break;
                }
                ...
                case Position_Sensor_DataType_sv_z_nAC:
                {
                    ...
                    if (registered_sensors[msg_rd->ind].type == Position_Sensor_Type_GlobalPositioning &&
                        registered_sensors[msg_rd->ind].frame != Position_Sensor_frame_SLAM && registered_sensors[msg_rd->ind].frame != Position_Sensor_frame_PSlamVFlu)
                    {
                        vector3<double> pos;
                        pos.x = msg_rd->posX;
                        pos.y = msg_rd->posY;
                        pos.z = msg_rd->posZ * 100;
                        PositionSensorUpdatePositionGlobalVel(msg_rd->ind, key, pos, vel,
                            available, msg_rd->delay,
                            msg_rd->trustXY, msg_rd->trustZ,
                            0,
                            msg_rd->LTTrustXY, msg_rd->LTTrustZ);
...
void (*const Mavlink_RC_Process[])( uint8_t Port_index , const mavlink_message_t* msg_sd ) = 
{
	...
	/*200-*/	0	,
	/*201-*/	0	,
	/*202-*/	Msg202_ACFlyHTL_IMU_REGISTER	,
	/*203-*/	Msg203_ACFlyHTL_IMU_UPDATE	,
	/*204-*/	0	,
	/*205-*/	0	,
	/*206-*/	0	,
	/*207-*/	0	,
