...
ModeResult M32_PosCtrl::main_func( void* param1, uint32_t param2 )
{
	...
	is_Attitude_Control_Enabled(&att_enabled);
	...
	//增稳
	double mthr_thrFilted = -1;
	int32_t mthr_prepCounter = 0;
	//避障
	bool AvLocked = false;
	...
	//打击模式
	StrikeCfg strikeCfg;
	ReadParamGroup( "strkeCfg", (uint64_t*)&strikeCfg, 0 );
	double hitAngle = -1000;
	double hitYawRateFilted = 0;
	double last_hitYaw = 0;
	uint16_t hit_lastCounter = 0;
	double hitSp = 100;
	...
	while(1)
	{
		...
		{	//指令更改模式
			...
			{	//mavlink定义模式
				...
				else if( main_mode==PX4_CUSTOM_MAIN_MODE_RATTITUDE )
				{
					reqMode = AFunc_Rattitude;
					msg_handled = 1;
				}
				else if( main_mode==PX4_CUSTOM_MAIN_MODE_MANUAL )
				{	//指令进入手动
					msg_handled = 1;
					reqMode = AFunc_Manual;
				}
				else if( main_mode==PX4_CUSTOM_MAIN_MODE_STABILIZED )
				{	//指令进入增稳
					msg_handled = 1;
					reqMode = AFunc_Stabilize;
				}
				...
				else if( (main_mode==PX4_CUSTOM_MAIN_MODE_AUTO || main_mode==0) && sub_mode==PX4_CUSTOM_SUB_MODE_AUTO_STRIKE )
				{	//跟随模式
					reqMode = AFunc_Strike;
					msg_handled = 1;
				}
			}
		}
						...
						{	//等待偏航旋转开始航点飞行
							...
							{						
								...
								else
								{
									...
									Position_Control_set_XYAutoSpeed(restoreWpInf.vel);
									MissionMode_BackToLastWp = 255;
								}
							}
							break;
...
//						}
		...
		else if( cMode==AFunc_Offboard )
		{
			...
			{	
				...
				if( Process_InflightCmd( msg.cmd, msg.params ) )
					msg_handled = 1;
				else if( check_NavCmd( msg.cmd, freq, msg.frame, msg.params ) )
				{	//指令可被执行
					init_NavCmdInf(&navInf);
				...
				{	//SET_POSITION_TARGET_LOCAL_NED
					...
					for(uint8_t i=0;i<12;++i)
						vv[i] = msg.params[i];
					SDLog_Msg_DebugVect("1084",vv,12);
					...
					{	//pva
						...
						switch( msg.frame )
						{
							...
							{
								Position_Control_set_TargetPosVelAccZ_SLAM_OffBoard( default_vslam_sensor_index, -posz, -velz, -accz );
								break;
							}
					...
					{	//va
						...
						switch( msg.frame )
						{
							case MAV_FRAME_VISION_NED:
							{
								Position_Control_set_TargetVelAccZ_SLAM_OffBoard( default_vslam_sensor_index, -velz, -accz );
								break;
							}
							case MAV_FRAME_VISION_ENU:
							{
					...
					{	//pva
						...
						switch( msg.frame )
						{
							case MAV_FRAME_VISION_NED:
							{
								Position_Control_set_TargetPosVelAccXY_SLAM_OffBoard( default_vslam_sensor_index, posy, posx, vely, velx, accy, accx );
								break;
							}
							case MAV_FRAME_VISION_ENU:
							{
								Position_Control_set_TargetPosVelAccXY_SLAM_OffBoard( default_vslam_sensor_index, posx, posy, velx, vely, accx, accy );
								break;
							}
							case MAV_FRAME_LOCAL_NED:
							{
					...
					{	//va
						...
						switch( msg.frame )
						{
							case MAV_FRAME_VISION_NED:
							{
								Position_Control_set_TargetVelAccXY_SLAM_OffBoard( default_vslam_sensor_index, vely, velx, accy, accx );
								break;
							}
							case MAV_FRAME_VISION_ENU:
							{
								Position_Control_set_TargetVelAccXY_SLAM_OffBoard( default_vslam_sensor_index, velx, vely, accx, accy );
								break;
							}
							case MAV_FRAME_LOCAL_NED:
							{
				...
				{	//SET_POSITION_TARGET_GLOBAL_INT
					...
					for(uint8_t i=0;i<12;++i)
						vv[i] = msg.params[i];
					SDLog_Msg_DebugVect("1086",vv,12);
		...
		{	//打击模式
			if( mode_switched )
			{
				...
				hitSensor sensor;
				if( get_hitSensor(&sensor) && sensor.available )
				{	//初始化状态
					ReadParamGroup( "strkeCfg", (uint64_t*)&strikeCfg, 0 );
					
					Quaternion rotQuatPit( cos(0.5*sensor.angle.x), 0, sin(0.5*sensor.angle.x), 0 );
					Quaternion rotQuatYaw( cos(0.5*sensor.angle.y), 0, 0, sin(0.5*sensor.angle.y) );
					Quaternion rotQuatRP = (sensor.attQuat.get_RP_quat()*sensor.camQuat)*rotQuatPit*rotQuatYaw;
					
					hitSp = 0;
					hitAngle = -asin(rotQuatRP.rotate_axis_x().z);
					last_hitYaw = sensor.angle.y;
					hitYawRateFilted = 0;
					hit_lastCounter = 0;
					setCurrentFlyMode(AFunc_Strike);
				}
				else
				{
					swManualMode;
			...
			if( inFlight )
			{
				...
				if( get_hitSensor(&sensor) && sensor.available )
				{
					...
					Quaternion rotQuatPit( cos(0.5*sensor.angle.x), 0, sin(0.5*sensor.angle.x), 0 );
					Quaternion rotQuatYaw( cos(0.5*sensor.angle.y), 0, 0, sin(0.5*sensor.angle.y) );
					Quaternion camQuat = sensor.attQuat*sensor.camQuat;
					Quaternion rotQuat = camQuat*rotQuatPit*rotQuatYaw;
					Quaternion rotQuatRP = (sensor.attQuat.get_RP_quat()*sensor.camQuat)*rotQuatPit*rotQuatYaw;
							
					//求最大允许倾角
					const double maxZAngle = 66 - 10;
					const double maxYAngle = 40 - 10;
					vector3<double> rotRol = rotQuatRP.rotate_axis_z();
					double rotRolScale = vector3<double>(1,0,0) * rotRol;
					if(!is_zero(rotRolScale))
						rotRolScale = 1.0 / rotRolScale;
					else
						rotRolScale = 1e9;
					vector3<double> rotPit = rotQuatRP.rotate_axis_y();
					double rotPitScale = vector3<double>(0,1,0) * rotPit;
					if(!is_zero(rotPitScale))
						rotPitScale = 1.0 / rotPitScale;
					else
						rotPitScale = 1e9;
					
					//求角度余量
					double rolFreeAngleL = ( degree2rad(0.5*maxZAngle) - sensor.angle.y ) * rotRolScale;
					double rolFreeAngleR = ( degree2rad(-0.5*maxZAngle) - sensor.angle.y ) * rotRolScale;
					double pitFreeAngleD = ( degree2rad(0.5*maxYAngle) - sensor.angle.x ) * rotPitScale;
					double pitFreeAngleU = ( degree2rad(-0.5*maxYAngle) - sensor.angle.x ) * rotPitScale;
					//最大允许倾角
					double minPit, maxPit;
					if( pitFreeAngleD > pitFreeAngleU )
					{
						double pit = quat.getPitch();
						minPit = pit - pitFreeAngleD;
						maxPit = pit - pitFreeAngleU;
					}
					else
					{
						double pit = quat.getPitch();
						minPit = pit - pitFreeAngleU;
						maxPit = pit - pitFreeAngleD;
					}
					double minRol, maxRol;
					if( rolFreeAngleR > rolFreeAngleL )
					{
						double rol = quat.getRoll();
						minRol = rol - rolFreeAngleR;
						maxRol = rol - rolFreeAngleL;
					}
					else
					{
						double rol = quat.getRoll();
						minRol = rol - rolFreeAngleL;
						maxRol = rol - rolFreeAngleR;
					}
					
					//期望速度
					const double tSp = strikeCfg.hitSp[0];
					//速度修正P
					const double velCorrP = strikeCfg.velCorrKp[0];
					//速度->角度Kp
					const double velAngleKp = degree2rad((double)strikeCfg.velAngleKp[0]);
					//俯仰打击角度
					const double pitHitAngle = strikeCfg.hitPitOffset[0];
					//打击速度
					hitSp += strikeCfg.hitSpFilter[0] * h * (tSp - hitSp);
					
					//控制航向
					double yawCtrolScale = vector3<double>(0,0,1) * rotRol;
					double targetYawRate = yawCtrolScale*strikeCfg.yawKp[0]*sensor.angle.y;
					Attitude_Control_set_Target_YawRate(targetYawRate);
					
					//x速度修正
					double corrVel_x = constrain( (degree2rad(pitHitAngle)-sensor.angle.x)*velCorrP, tSp );
					//double corrVel_x = constrain( ( hitAngle - -asin(rotQuatRP.rotate_axis_x().z) )*velCorrP, tSp );
					vector3<double> corrVelX = camQuat.rotate_axis_z() * corrVel_x;
					
					//y速度修正
					double rolCtrolScale = vector3<double>(1,0,0) * rotRol;
					double corrVel_y = constrain( rolCtrolScale*sensor.angle.y*velCorrP, tSp );
					vector3<double> corrVelY = camQuat.rotate_axis_y() * corrVel_y;
					
					//计算期望速度
					vector3<double> tVel = rotQuat.rotate_axis_x() * hitSp;
					tVel += corrVelX + corrVelY;
					
					//速度转换到flu
					vector3<double> tVel_flu;
					double sin_Yaw, cos_Yaw;
					fast_sin_cos( yaw, &sin_Yaw, &cos_Yaw );
					tVel_flu.x = ENU2BodyHeading_x( tVel.x , tVel.y , sin_Yaw , cos_Yaw );
					tVel_flu.y = ENU2BodyHeading_y( tVel.x , tVel.y , sin_Yaw , cos_Yaw );
					tVel_flu.z = tVel.z;
					
					//限幅
					vector2<double> ctrl_rolPit;
					ctrl_rolPit.x = -tVel_flu.y * velAngleKp;
					ctrl_rolPit.y = tVel_flu.x * velAngleKp;
					vector2<double> ctrl_rolPit_c;
					ctrl_rolPit_c.x = constrain( ctrl_rolPit.x, minRol, maxRol );
					ctrl_rolPit_c.y = constrain( ctrl_rolPit.y, minPit, maxPit );
					
					//控制垂直速度
					Position_Control_set_TargetVelocityZ( tVel_flu.z );
					...
					double vv[15];
					vv[0] = rad2degree(sensor.angle.x);
					vv[1] = rad2degree(sensor.angle.y);
					vv[2] = corrVel_x;
					vv[3] = corrVel_y;
					vv[4] = tVel_flu.x;
					vv[5] = tVel_flu.y;
					vv[6] = tVel_flu.z;
					vv[7] = rad2degree(-tVel_flu.y * velAngleKp);
					vv[8] = rad2degree(tVel_flu.x * velAngleKp);
					vv[9] = rad2degree(minPit);
					vv[10] = rad2degree(maxPit);
					vv[11] = rad2degree(pitFreeAngleD);
					vv[12] = rad2degree(pitFreeAngleU);
					vv[13] = rad2degree(hitAngle);
					vv[14] = rad2degree(asin(rotQuatRP.rotate_axis_x().z));
					SDLog_Msg_DebugVect("daji",vv,15);
				}
				else
				{
					if( hit_lastCounter >= 2*freq )
					{
						swManualMode;
					}
					else
					{
						...
						double velZ;
						Position_Control_get_TargetVelocityZ(&velZ);
						Position_Control_set_TargetVelocityZ(velZ);
			...
			{	//未起飞
				...
				Attitude_Control_set_Target_RollPitchRate(0,0);
				Attitude_Control_set_Target_YawRate(0);
			}
		}
		else
...
//				followOrigin.zero();
			...
			{	//刚进入手动模式初始化变量
				...
				//增稳
				double targetThr = 0;
				get_Target_Throttle(&mthr_thrFilted);
				mthr_prepCounter = 0;
			}
			...
			{				
				...
				//切换定高定点
				if( cMode == AFunc_Manual || cMode==AFunc_Stabilize || cMode==AFunc_Rattitude )
...
//									else
	...
	//						);
				...
				{	//摇杆不在中间手动飞行
					...
					{	//手动控制油门
						double targetThr = rc.data[0];
						if( abs(mthr_prepCounter) >= 100000 )
							Attitude_Control_set_Throttle(targetThr);
						else
						{
							if( abs(mthr_prepCounter) >= 2.5*freq )
								mthr_prepCounter = 1000000;
							
							if( targetThr>mthr_thrFilted && mthr_prepCounter>=0 )
								++mthr_prepCounter;
							else if( targetThr<mthr_thrFilted && mthr_prepCounter<=0 )
								--mthr_prepCounter;
							else
								mthr_prepCounter = 1000000;
								
							mthr_thrFilted += 1.0*h * (targetThr - mthr_thrFilted);
							Attitude_Control_set_Throttle(mthr_thrFilted);
						}
					}
					...
					else
					{
						...
						//俯仰横滚杆控俯仰横滚
						if( cMode==AFunc_Rattitude )
						{
							double RPCtrlScale = degree2rad( getAttCtrlCfg()->maxRPSp[0] / 50.0 );
							double outRol = remove_deadband( rc.data[3] - 50, MFunc_cfg.ratitudeZone[0] );
							bool outRolZero = (outRol == 0);
							outRol *= 1.0 / (50 - MFunc_cfg.NeutralZone[0]);
							outRol = sign(outRol) * pow( fabs(outRol), MFunc_cfg.RolRateE[0] );
							outRol *= 50;
							double outPit = remove_deadband( rc.data[2] - 50, MFunc_cfg.ratitudeZone[0] );
							bool outPitZero = (outPit == 0);
							outPit *= 1.0 / (50 - MFunc_cfg.NeutralZone[0]);
							outPit = sign(outPit) * pow( fabs(outPit), MFunc_cfg.PitRateE[0] );
							outPit *= 50;
							if( outRolZero && outPitZero )
							{	//半自稳回平
								Attitude_Control_set_Target_RollPitch( 
									remove_deadband( rc.data[3] - 50, MFunc_cfg.NeutralZone[0] )*RPCtrlScale,
									remove_deadband( rc.data[2] - 50, MFunc_cfg.NeutralZone[0] )*RPCtrlScale
								);
							}
							else
							{
								Attitude_Control_set_Target_RollPitchRate( 
									outRol*RPCtrlScale,
									outPit*RPCtrlScale
								);
							}
						}
						else if( cMode==AFunc_Manual )
						{
							double RPCtrlScale = degree2rad( getAttCtrlCfg()->maxRPSp[0] / 50.0 );
							double outRol = remove_deadband( rc.data[3] - 50, MFunc_cfg.NeutralZone[0] );
							outRol *= 1.0 / (50 - MFunc_cfg.NeutralZone[0]);
							outRol = sign(outRol) * pow( fabs(outRol), MFunc_cfg.RolRateE[0] );
							outRol *= 50;
							double outPit = remove_deadband( rc.data[2] - 50, MFunc_cfg.NeutralZone[0] );
							outPit *= 1.0 / (50 - MFunc_cfg.NeutralZone[0]);
							outPit = sign(outPit) * pow( fabs(outPit), MFunc_cfg.PitRateE[0] );
							outPit *= 50;
							Attitude_Control_set_Target_RollPitchRate( 
	...
	//						);
					...
					//偏航杆在中间锁偏航
					//不在中间控制偏航速度
					if( cMode == AFunc_Manual )
					{
						double YCtrlScale = degree2rad( getAttCtrlCfg()->maxYSp[0] / 50.0 );
						double out = remove_deadband( 50 - rc.data[1], MFunc_cfg.NeutralZone[0] );
						out *= 1.0 / (50 - MFunc_cfg.NeutralZone[0]);
						out = sign(out) * pow( fabs(out), MFunc_cfg.YawRateE[0] );
						out *= 50;
						Attitude_Control_set_Target_YawRate( out*YCtrlScale );
					}
					else
					{
						double YCtrlScale = degree2rad( getAttCtrlCfg()->maxYSp[0] / 50.0 );
			...
			{	//失控
				// 失控后一段时间再执行
				// 防止遥控器间歇失控导致操控问题
				if( noRcStartTIME.get_pass_time() > 0.5 )
				{
					//无遥控信号进入安全模式
					Altitude_Control_Enable();
					Position_Control_Enable();				
