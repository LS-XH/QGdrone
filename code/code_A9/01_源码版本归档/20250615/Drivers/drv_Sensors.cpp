	...
	static bool AK8975_init()
	{
		/*AK8975初始化*/
			...
			/*读AK8975校准数据*/
				...
				AK8975_key = IMUMagnetometerRegister( Internal_Magnetometer_Index, "AK8975", 0.003, 0.1 );
			/*读AK8975校准数据*/
			return AK8975_key!=0;
		/*AK8975初始化*/
	}
	...
	*/
	...
	/*BMI088初始化*/
		/*加速度计初始化*/
			...
			bmi088_accel_key = IMUAccelerometerRegister( 0, "BMI088", GravityAcc / 1365.0, 0.1 );
		/*加速度计初始化*/
		...
		/*陀螺初始化*/
			...
			bmi088_gyro_key = IMUGyroscopeRegister( 0, "BMI088", 0.00106526443603169529841533860381, 0.1 );
...
static void BMI088GyroTCB( void *pvParameter1, uint32_t ulParameter2 )
{
	...
	if( data.x == std::numeric_limits<int16_t>::min() || data.y == std::numeric_limits<int16_t>::min() || data.z == std::numeric_limits<int16_t>::min() || \
		...
		data_error = true;
	//更新传感器数据
	IMUGyroscopeUpdate( 0,bmi088_gyro_key, data, data_error, 0.01 );		
...
{	
	...
	if( data.x == std::numeric_limits<int16_t>::min() || data.y == std::numeric_limits<int16_t>::min() || data.z == std::numeric_limits<int16_t>::min() || \
		...
		data_error = true;
	//更新传感器数据
	IMUAccelerometerUpdate( 0,bmi088_accel_key, data, data_error, 0.01 );
	MS_Notify_IMUAccelUpdate(0);
}
...
static void AK8975TCB( void *pvParameter1, uint32_t ulParameter2 )
{
	...
	//更新传感器数据
	IMUMagnetometerUpdate( Internal_Magnetometer_Index,AK8975_key, data, false, 0.01 );
}
