...
/*外置罗盘定义*/
	...
	/*     名称     , iic地址, 数据寄存器地址 , 高位在前, 手动采样 , 采样寄存器地址, 采样设置, ID 地址 , ID   ,      配置寄存器地址      , 配置数目,  配置                                                     ,  灵敏度      , 采样率毫秒  , 轴向                 ,  */
		{ "HMC5983ext"   , 0x1e   ,      3         ,    true ,   false  ,            0  ,      0  ,      0  , 0x10 ,      { 0, 1, 2 }         , 3       , { (1<<7) | (0<<5) | (7<<2) | (0<<0) , (1<<5) , 0 }        , 0.00092      , 5           , { -3 , -1 , -2 }   } , //HMC5983
		{ "HMC5883ext"   , 0x1e   ,      3         ,    true ,   false  ,            0  ,      0  ,      0  , 0xf0 ,      { 0, 1, 2 }         , 3       , { (0<<7) | (0<<5) | (6<<2) | (0<<0) , (1<<5) , 0 }        , 0.00092      , 20          , { -3 , -1 , -2 }   } , //HMC5883
		{ "QMC5883ext"   , 0x0d   ,      0         ,   false ,   false  ,            0  ,      0  ,   0x0d  , 0xff ,      { 0x9, 0xa, 0xb }   , 3       , { (0<<6) | (0<<4) | (0b10<<2) | (1<<0) , (1<<6) , 1 }     , 8.33333333e-5, 10          , { -2 , -1 , -3 }   } , //HMC5883
		{ "Mag3110ext"   , 0x0e   ,      1         ,    true ,   false  ,            0  ,      0  ,      7  , 0xc4 ,      { 0x10 }            , 1       , { (0<<5) | (0<<3) | (0<<2) | (0<<1) | (1<<0) }            , 0.001        , 10          , { -2 , -1 , 3 }    } , //Mag3110
		{ "IST8310ext"   , 0x0e   ,      3         ,   false ,    true  ,          0xa  ,      1  ,      0  , 0x10 ,      { 0x41, 0x42 }      , 2       , { 0x24 , 0xc0 }                                           , 0.003        , 30          , { -1 , -2 , -3 }    } , //IST8310
	};
