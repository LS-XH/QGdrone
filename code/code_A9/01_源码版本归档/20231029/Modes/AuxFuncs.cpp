...
/*拍照功能*/
		...
		if( xSemaphoreTakeRecursive( CamMutex, 0.1*configTICK_RATE_HZ ) )
		{
			...
			{	//无相机返回
				...
				{//数字云台需要拍照
					...
					SendCmdMsgToTask( cmd_msg, CAMERA_GIMBAL_CONTROL_CMD_QUEUE_ID, 0.002 );
					...
					{	//数字云台等待指令回馈
						...
						{
							CmdMsg msg;
							bool msg_rd = TaskReceiveCmdMsg( &msg, CAMERA_GIMBAL_FEEDBACK_QUEUE_ID, 0.002 );
							if( msg_rd )
							{	//相机反馈成功
								res = msg.cmd;
								break;
	...
	static void AuxCamTakePhotoAsyncServer(float h, const AuxFuncsConfig& aux_configs)
	{
			...
			)
		{
			...
			//清空数字云台反馈缓冲区
			CmdMsg msg;
			TaskReceiveCmdMsg( &msg, CAMERA_GIMBAL_FEEDBACK_QUEUE_ID, 0 );
		...
		{	//空闲时处理数字云台拍照反馈
			CmdMsg msg;
			bool msg_rd = TaskReceiveCmdMsg( &msg, CAMERA_GIMBAL_FEEDBACK_QUEUE_ID, 0 );
			if( msg_rd && msg.cmd==1 )
			{
				xSemaphoreTake( PosLogMutex, portMAX_DELAY );
					++PhotoCnt;
