#include "drv_GPS1_UbloxBase.hpp"
...
GPS_Present:
        ...
        if (ReadParamGroup(SName(gpsName) + SName("Cfg"), (uint64_t *)&gps_cfg, 0) == PR_OK)
        {
            ...
            sensor_key = PositionSensorRegister(gps_sensor_index,
                SName(gpsName) + SName("_UbloxBase"),
            ...
            if (driver_info.port.read(&r_data, 1, 2, 0.1))
            {
                ...
                if (pack_length)
                {
                    ...
                    { // UBX-NAV-PVT
                        ...
                        {
                            gps_setRtcOnce(pack->lat * 1e-7, pack->lon * 1e-7,
                                pack->year, pack->month, pack->day, pack->hour, pack->min, pack->sec);

                            uint64_t gps_time = gps_packUtc(pack->year, pack->month, pack->day,
                                pack->hour, pack->min, (double)pack->sec + (pack->iTOW % 1000) * 1e-3);
                        ...
                        if (gps_fix != 6 && xy_trustD < 100)
                            xy_trustD = 100;
                        if (!zHighPrec && z_trustD < 150)
                            z_trustD = 150;
...
void init_drv_GPS1_UbloxBase()
{
        PortFunc_Register(15, GPS_DriverInit);
}
