...
static int8_t send_init_msg(DriverInfo &driver_info, uint8_t gps_ind)
{
    ...
    int8_t COM = -1;
    len = sprintf(UM482Clr, "LOG BESTPOSA ONTIME 1\r\n");
    driver_info.port.write((uint8_t *)&UM482Clr[0], len, portMAX_DELAY, portMAX_DELAY);
    driver_info.port.wait_sent(1);
    os_delay(0.15);
    ...
    if (isGPSCfgAvailble)
    {
        ...
        {
            len = sprintf(UM482Clr, "MASK QZSS\r\n");
        }
    }
    else
        len = sprintf(UM482Clr, "UNMASK QZSS\r\n");
    ...
    // 接收机运动的航向
    len = sprintf(UM482Clr, "LOG HEADINGB ONTIME 0.1\r\n");
    driver_info.port.write((uint8_t *)&UM482Clr[0], len, portMAX_DELAY, portMAX_DELAY);
    driver_info.port.wait_sent(1);
    os_delay(0.1);

    // UTC 时间
    len = sprintf(UM482Clr, "LOG TIMEB ONTIME 1\r\n");
    driver_info.port.write((uint8_t *)&UM482Clr[0], len, portMAX_DELAY, portMAX_DELAY);
    driver_info.port.wait_sent(1);
    os_delay(0.1);
    ...
    len = sprintf(UM482Clr, "CONFIG COM%d 460800 8 n 1\r\n", COM - '0');
    driver_info.port.write((uint8_t *)&UM482Clr[0], len, portMAX_DELAY, portMAX_DELAY);
    driver_info.port.wait_sent(1);
    os_delay(0.1);
