"""远程 ZIP64 单文件提取器 v3 —— bicycle 版
基于 v2 改造：
  1. 目标场景参数化（SCENE 变量，默认 bicycle）
  2. 支持 SOCKS5 代理（默认 127.0.0.1:7897，可用环境变量 EXTRACT_PROXY 覆盖，none=直连）
...
"""
import struct, zlib, os, sys, time, socket

URL = "https://repo-sam.inria.fr/fungraph/3d-gaussian-splatting/datasets/pretrained/models.zip"
ZIP_SIZE = 14660630999          # 已知总大小
SCENE = os.environ.get("EXTRACT_SCENE", "bicycle")
ITER = os.environ.get("EXTRACT_ITER", "30000")   # 训练迭代次数（30000 最终版 / 7000 早期版）
PROXY = os.environ.get("EXTRACT_PROXY", "socks5://127.0.0.1:7897")
...
def setup_proxy():
    ...
    try:
        ...
        print("WARN: PySocks 未安装，将走直连；建议: pip install pysocks")
        return
    host, port = PROXY.split("://")[1].split(":")
    socks.set_default_proxy(socks.SOCKS5, host, int(port))
    socket.socket = socks.socksocket
    print(f"SOCKS5 代理已启用: {PROXY}")
...
def main():
    ...
    print(f"目标场景: {SCENE} | ZIP: {ZIP_SIZE/1e9:.1f}GB")
    ...
    if LIST_ONLY:
        print(f"\n共 {len(targets)} 个 PLY：")
        for t in targets:
            print(f"  {t[0]}  comp={t[2]/1e6:.0f}MB → uncomp={t[3]/1e6:.0f}MB")
        print("'--list' 模式结束")
        sys.exit(0)

    # 优先目标场景的最终模型(point_cloud + 指定 iteration)
    chosen = None
    for t in targets:
        p = t[0].lower()
        if SCENE.lower() in p and 'point_cloud' in p and f'iteration_{ITER}' in p:
            chosen = t
            break
    if chosen is None:
        chosen = targets[0]
        print(f"WARN: 未找到 {SCENE}/point_cloud/iteration_{ITER}，退回第一个 PLY")
