...
static int8_t send_init_msg(DriverInfo &driver_info)
{
    int len = 0;
    char UM982Clr[100];
    ...
    // 从天线计算出的最佳可用位置和速度Bestnav
    len = sprintf(UM982Clr, "BESTNAVHB 0.05\r\n");
    driver_info.port.write((uint8_t *)&UM982Clr[0], len, portMAX_DELAY, portMAX_DELAY);
    driver_info.port.wait_sent(1);
    os_delay(0.1);
...
GPS_CheckBaud:
    while (1)
    {
        ...
        // 发送配置
        if (send_init_msg(driver_info) < 0)
            continue;
        ...
        if (driver_info.port.read(&r_data, 1, 2, 0.1))
        {
            if (GPS_ParseByte(&gps_state, frame_datas, r_data))
            {
                ...
                { // BESTNAV 最佳位置和速度
                    ...
                    double rawHeight = pack->height;// + pack->undulation;
                    if (z_available)
                    {
                        if (vAcc > 16)
                            z_available = false;
                    ...
                    {
                        if (vAcc < 8 && pack->height != 0)
                        {
                            gps_alt = rawHeight * 100;
                            z_available = true;
                        }
                    }
